...
```
`.github/workflows/release.yml` が macOS（dmg/zip）・Windows（nsis）・Linux（AppImage/deb/rpm/pacman）をビルドし、GitHub Releases へ公開します。Linux の対応: Ubuntu=`.deb` / AlmaLinux=`.rpm` / Arch=`pacman`（AppImage は全 distro 共通の保険）。現状のビルドは**未署名**です。
...
npm test           # vitest (path-resolver unit tests)
npm run build      # electron-vite build -> out/
```
...
git push origin v0.1.0
```
`.github/workflows/release.yml` builds macOS (dmg/zip), Windows (nsis), and Linux (AppImage/deb/rpm/pacman) and publishes them to GitHub Releases. Linux mapping: Ubuntu=`.deb` / AlmaLinux=`.rpm` / Arch=`pacman` (AppImage is the universal fallback). Builds are currently **unsigned**.
